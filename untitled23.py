...
from sklearn.feature_selection import mutual_info_regression, RFE, SelectKBest, f_classif
...
class ClusteringOptimizer:
    ...
    def __init__(self, balance_tolerance: float = 0.15, random_state: int = 42, 
                 ...
                 min_features_per_cluster: int = 5, max_feature_correlation: float = 0.9,
        ...
        Args:
            ...
            min_features_per_cluster: Minimum features required for valid clustering (default: 5)
            max_feature_correlation: Maximum allowed correlation between features (default: 0.9)
            min_effect_size: Minimum effect size for feature inclusion (default: 0.2)
        """
        ...
        self.min_features_per_cluster = min_features_per_cluster
        self.max_feature_correlation = max_feature_correlation
        self.min_effect_size = min_effect_size
        
        # Feature weights for ensemble approach
    ...
    def preprocess_datasets(self, datasets: Dict[str, Dict]) -> Dict[str, Dict]:
        ...
        Now includes Sparse PCA, ICA, UMAP, t-SNE, and Varimax rotation.
        """
        ...
        # Extended dimensionality reduction options
        dim_reduction = {
            'none': None,
            'pca_90': ('pca', 0.90),
            'pca_95': ('pca', 0.95),
            'pca_varimax': ('pca_varimax', 0.95),  # NEW: PCA with varimax rotation
            'sparse_pca': ('sparse_pca', 10),      # NEW: Sparse PCA
            'ica': ('ica', 10),                    # NEW: Independent Component Analysis
            'kernel_pca': ('kernel_pca', 10),
            'umap': ('umap', 2),                   # NEW: UMAP
            'tsne': ('tsne', 2),                   # NEW: t-SNE
        }
        ...
        for dataset_name, dataset_info in datasets.items():
            ...
            X_selected, selected_feature_names = self.apply_feature_selection(X, feature_names)
            
            # Create enhanced features variant
            enhanced_X, enhanced_feature_names = self.enhance_features(X_selected, selected_feature_names, max_features=None)
            
            # Process both original and enhanced datasets
            for data_X, data_feature_names, suffix in [
                (X_selected, selected_feature_names, "_selected"),
                ...
                for scaler_name, scaler in scalers.items():
                    ...
                    for dim_name, dim_params in dim_reduction.items():
                        ...
                        if dim_params is not None:
                            ...
                            elif method == 'pca_varimax' and X_final.shape[1] > 2:
                                try:
                                    X_final, final_feature_names = self._apply_pca_varimax(X_scaled, param)
                                except:
                                    continue
                            
                            # Sparse PCA
                            elif method == 'sparse_pca' and X_final.shape[1] > 2:
                                try:
                                    n_components = min(param, X_scaled.shape[1], X_scaled.shape[0]-1)
                                    spca = SparsePCA(n_components=n_components, random_state=self.random_state,
                                                   alpha=1.0)  # Alpha controls sparsity
                                    X_final = spca.fit_transform(X_scaled)
                                    final_feature_names = [f'SPC{i+1}' for i in range(X_final.shape[1])]
                                except:
                                    continue
                            
                            # Independent Component Analysis
                            elif method == 'ica' and X_final.shape[1] > 2:
                                try:
                                    n_components = min(param, X_scaled.shape[1])
                                    ica = FastICA(n_components=n_components, random_state=self.random_state)
                                    X_final = ica.fit_transform(X_scaled)
                                    final_feature_names = [f'IC{i+1}' for i in range(X_final.shape[1])]
                                except:
                                    continue
                            
                            # UMAP
                            elif method == 'umap' and UMAP_AVAILABLE and X_final.shape[1] > 2:
                                try:
                                    reducer = umap.UMAP(n_components=param, random_state=self.random_state,
                                                      n_neighbors=min(15, X_scaled.shape[0]-1))
                                    X_final = reducer.fit_transform(X_scaled)
                                    final_feature_names = [f'UMAP{i+1}' for i in range(X_final.shape[1])]
                                except:
                                    continue
                            
                            # t-SNE
                            elif method == 'tsne' and X_final.shape[1] > 2:
                                try:
                                    tsne = TSNE(n_components=param, random_state=self.random_state,
                                              perplexity=min(30, X_scaled.shape[0]-1))
                                    X_final = tsne.fit_transform(X_scaled)
                                    final_feature_names = [f'tSNE{i+1}' for i in range(X_final.shape[1])]
                                except:
                                    continue
    ...
    def apply_feature_selection(self, X: np.ndarray, feature_names: List[str], 
                               max_features: Optional[int] = None) -> Tuple[np.ndarray, List[str]]:
        """
        Apply advanced feature selection methods including mutual information,
        LASSO, and recursive feature elimination.
        """
        print("  Applying advanced feature selection...")
        
        if max_features is None:
            max_features = min(X.shape[1], 50)  # Default to top 50 features
        
        # Initialize selection scores
        n_features = X.shape[1]
        selection_scores = np.zeros((4, n_features))  # 4 methods
        
        # 1. Variance-based selection (existing)
        variances = np.var(X, axis=0)
        selection_scores[0] = variances / np.max(variances) if np.max(variances) > 0 else variances
        
        # 2. Mutual Information selection
        try:
            # Create pseudo-labels based on initial clustering
            kmeans_init = KMeans(n_clusters=2, random_state=self.random_state, n_init=3)
            pseudo_labels = kmeans_init.fit_predict(X)
            mi_scores = mutual_info_regression(X, pseudo_labels, random_state=self.random_state)
            selection_scores[1] = mi_scores / np.max(mi_scores) if np.max(mi_scores) > 0 else mi_scores
        except:
            selection_scores[1] = selection_scores[0]  # Fallback to variance
        
        # 3. LASSO-based selection
        try:
            lasso_cv = LassoCV(cv=5, random_state=self.random_state, max_iter=5000)
            lasso_cv.fit(X, pseudo_labels)
            lasso_importance = np.abs(lasso_cv.coef_)
            selection_scores[2] = lasso_importance / np.max(lasso_importance) if np.max(lasso_importance) > 0 else lasso_importance
        except:
            selection_scores[2] = selection_scores[0]  # Fallback to variance
        
        # 4. Random Forest importance
        try:
            rf = RandomForestClassifier(n_estimators=100, random_state=self.random_state)
            rf.fit(X, pseudo_labels)
            selection_scores[3] = rf.feature_importances_
        except:
            selection_scores[3] = selection_scores[0]  # Fallback to variance
        
        # Combine scores with weights
        weights = [0.2, 0.3, 0.3, 0.2]  # Emphasize MI and LASSO
        combined_scores = np.average(selection_scores, axis=0, weights=weights)
        
        # Update feature weights for ensemble approach
        for i, fname in enumerate(feature_names):
            if fname not in self.feature_weights:
                self.feature_weights[fname] = []
            self.feature_weights[fname].append(combined_scores[i])
        
        # Select top features
        top_indices = np.argsort(combined_scores)[::-1][:max_features]
        
        # Remove highly correlated features
        selected_indices = self._remove_correlated_features(X, top_indices)
        
        # Ensure minimum features
        if len(selected_indices) < self.min_features_per_cluster:
            selected_indices = top_indices[:self.min_features_per_cluster]
        
        X_selected = X[:, selected_indices]
        selected_feature_names = [feature_names[i] for i in selected_indices]
        
        print(f"  Selected {len(selected_indices)} features from {n_features}")
        
        return X_selected, selected_feature_names
    
    def _apply_pca_varimax(self, X: np.ndarray, variance_threshold: float) -> Tuple[np.ndarray, List[str]]:
        """Apply PCA followed by Varimax rotation to distribute variance more evenly."""
        # First apply PCA
        ...
        # Normalize
        X_normalized = zscore(X_pca)
        
        # Varimax rotation
        n_comp = X_pca.shape[1]
        rotation_matrix = np.eye(n_comp)
        
        # Iterative rotation
        for _ in range(100):  # Max iterations
            comp_rot = X_normalized @ rotation_matrix
            u, s, v = svd(X_normalized.T @ (comp_rot ** 3 - comp_rot @ np.diag(np.mean(comp_rot ** 2, axis=0))))
            rotation_matrix = u @ v
            
        X_rotated = X_pca @ rotation_matrix
        
        feature_names = [f'VPC{i+1}' for i in range(X_rotated.shape[1])]
        return X_rotated, feature_names
    
    def _remove_correlated_features(self, X: np.ndarray, indices: np.ndarray) -> np.ndarray:
        """Remove highly correlated features to ensure diversity."""
        ...
        while remaining:
            # Take the first remaining feature
            current = remaining[0]
            selected.append(current)
            remaining.remove(current)
            
            # Remove features highly correlated with current
            current_idx = list(indices).index(current)
            to_remove = []
            
            for other in remaining:
                other_idx = list(indices).index(other)
                if abs(corr_matrix[current_idx, other_idx]) > self.max_feature_correlation:
    ...
    def enhance_features(self, X: np.ndarray, feature_names: List[str], 
        ...
        # Select features with highest variance and effect sizes
        ...
        for i in range(X.shape[1]):
            group0 = X[pseudo_labels == 0, i]
            group1 = X[pseudo_labels == 1, i]
            
            if len(group0) > 1 and len(group1) > 1:
                pooled_std = np.sqrt((np.var(group0) * (len(group0) - 1) + 
                                    np.var(group1) * (len(group1) - 1)) / 
                                    (len(group0) + len(group1) - 2))
                if pooled_std > 0:
                    d = np.abs(np.mean(group1) - np.mean(group0)) / pooled_std
        ...
        # Combine variance and effect size for feature selection
        combined_scores = variances * effect_sizes
        top_indices = np.argsort(combined_scores)[::-1]
        
        # Filter by minimum effect size
        top_variance_indices = [idx for idx in top_indices 
                               if effect_sizes[idx] >= self.min_effect_size][:min(5, X.shape[1])]
        ...
        nonlinear_count = 0
        max_nonlinear = min(10, len(top_variance_indices) * 2)
        ...
        # 2. Add pairwise feature interactions (only for high effect size features)
        interaction_count = 0
        max_interactions = 10
    ...
    # MODIFIED - Added feature diversity objective
    def genetic_algorithm_optimizer(self, dataset_info, n_generations=30, population_size=50):
        ...
        Now includes multi-objective optimization for feature diversity.
        """
        ...
        def fitness_function(individual):
            ...
            preprocessing_options = [
                ...
                {'scaler': 'standard', 'sparse_pca': 10},  # NEW
                {'scaler': 'robust', 'ica': 10}            # NEW
            ]
            ...
            if np.sum(subject_mask) < self.min_subjects or np.sum(feature_mask) < self.min_features_per_cluster:
                return (-1, 0)  # Two objectives: silhouette and feature diversity
            ...
            if silhouette > 0:
                X_subset = X[subject_mask][:, feature_mask]
                # Correlation-based diversity
                corr_matrix = np.corrcoef(X_subset.T)
                avg_correlation = np.mean(np.abs(corr_matrix[np.triu_indices_from(corr_matrix, k=1)]))
                diversity_score = 1 - avg_correlation  # Higher diversity = lower correlation
            else:
                diversity_score = 0
            
            # Return multi-objective fitness
            return (silhouette, diversity_score)
        
        # Set up DEAP for multi-objective optimization
        if hasattr(creator, "FitnessMulti"):
            del creator.FitnessMulti
        if hasattr(creator, "Individual"):
            del creator.Individual
            
        creator.create("FitnessMulti", base.Fitness, weights=(1.0, 0.3))  # Weight silhouette more
        ...
        for generation in tqdm(range(NGEN), desc="GA generations"):
            ...
            # Track best fitness
            fits = [ind.fitness.values[0] for ind in population]  # Primary objective
        ...
        # Get best individual (highest silhouette with good diversity)
        # Sort by silhouette first, then by diversity
        population.sort(key=lambda x: (x.fitness.values[0], x.fitness.values[1]), reverse=True)
        ...
        preprocessing_options = [
            ...
            {'scaler': 'standard', 'pca_variance': 0.95},
            {'scaler': 'robust', 'pca_variance': 0.90},
            {'scaler': 'standard', 'sparse_pca': 10},
            {'scaler': 'robust', 'ica': 10}
        ]
    ...
    # UNCHANGED - Keep existing implementation but add new preprocessing options
    def bayesian_optimization(self, dataset_info, n_trials=100):
        """[MODIFIED - Added new preprocessing options to search space]"""
        ...
        def objective(trial):
            ...
            # Ensure at least minimum features
            if np.sum(feature_mask) < self.min_features_per_cluster:
                feature_mask[np.random.choice(n_features, self.min_features_per_cluster)] = True
            ...
            # Sample preprocessing parameters with new options
            preprocessing_params = {}
            preprocessing_params['scaler'] = trial.suggest_categorical('scaler', 
                [None, 'standard', 'robust'])
            
            dim_reduction = trial.suggest_categorical('dim_reduction', 
                ['none', 'pca', 'sparse_pca', 'ica', 'umap', 'tsne'])
            
            if dim_reduction == 'pca':
                preprocessing_params['pca_variance'] = trial.suggest_float('pca_variance', 0.8, 0.99)
            elif dim_reduction == 'sparse_pca':
                preprocessing_params['sparse_pca'] = trial.suggest_int('sparse_pca_components', 5, 20)
            elif dim_reduction == 'ica':
                preprocessing_params['ica'] = trial.suggest_int('ica_components', 5, 20)
            elif dim_reduction == 'umap' and UMAP_AVAILABLE:
                preprocessing_params['umap'] = 2
            elif dim_reduction == 'tsne':
                preprocessing_params['tsne'] = 2
    ...
    # MODIFIED - Added new preprocessing options
    def random_search_optimizer(self, dataset_info, n_iterations=1000):
        """
        Random search over configurations.
        Now includes new preprocessing methods and feature diversity.
        """
        ...
        preprocessing_options = [
            ...
            {'scaler': 'robust', 'pca_variance': 0.90},
            {'scaler': 'standard', 'sparse_pca': 10},      # NEW
            {'scaler': 'robust', 'sparse_pca': 10},        # NEW
            {'scaler': 'standard', 'ica': 10},             # NEW
            {'scaler': 'robust', 'ica': 10},               # NEW
            {'scaler': 'standard', 'pca_varimax': 0.95},   # NEW
        ]
        
        if UMAP_AVAILABLE:
            preprocessing_options.extend([
                {'scaler': 'standard', 'umap': 2},
                {'scaler': 'robust', 'umap': 2},
            ])
        
        preprocessing_options.extend([
            {'scaler': 'standard', 'tsne': 2},
            {'scaler': 'robust', 'tsne': 2},
        ])
        ...
        for iteration in tqdm(range(n_iterations), desc="Random search"):
            ...
            # Random feature inclusion (ensure minimum)
            n_features_to_include = np.random.randint(self.min_features_per_cluster, n_features + 1)
    ...
    # UNCHANGED - Metric calculation methods
    ...
    # MODIFIED - Added support for new preprocessing methods
    def evaluate_configuration(self, dataset_X, subject_mask, feature_mask, 
                              clustering_params, preprocessing_params, min_features=None):
        """
        Evaluate a specific configuration and return silhouette score and balance.
        Now supports new preprocessing methods and enforces constraints.
        """
        if min_features is None:
            min_features = self.min_features_per_cluster
        ...
        try:
            ...
            elif preprocessing_params.get('pca_varimax'):
                X_processed, _ = self._apply_pca_varimax(X_processed, preprocessing_params['pca_varimax'])
                if X_processed.shape[1] < 2:
                    return -1, 0, X_subset.shape[0], {}
            
            # Apply Sparse PCA
            elif preprocessing_params.get('sparse_pca'):
                n_components = min(preprocessing_params['sparse_pca'], X_processed.shape[1], X_processed.shape[0]-1)
                spca = SparsePCA(n_components=n_components, random_state=self.random_state, alpha=1.0)
                X_processed = spca.fit_transform(X_processed)
                if X_processed.shape[1] < 2:
                    return -1, 0, X_subset.shape[0], {}
            
            # Apply ICA
            elif preprocessing_params.get('ica'):
                n_components = min(preprocessing_params['ica'], X_processed.shape[1])
                ica = FastICA(n_components=n_components, random_state=self.random_state)
                X_processed = ica.fit_transform(X_processed)
                if X_processed.shape[1] < 2:
                    return -1, 0, X_subset.shape[0], {}
            
            # Apply UMAP
            elif preprocessing_params.get('umap') and UMAP_AVAILABLE:
                reducer = umap.UMAP(n_components=preprocessing_params['umap'], 
                                  random_state=self.random_state,
                                  n_neighbors=min(15, X_processed.shape[0]-1))
                X_processed = reducer.fit_transform(X_processed)
            
            # Apply t-SNE
            elif preprocessing_params.get('tsne'):
                tsne = TSNE(n_components=preprocessing_params['tsne'], 
                           random_state=self.random_state,
                           perplexity=min(30, X_processed.shape[0]-1))
                X_processed = tsne.fit_transform(X_processed)
    ...
    # UNCHANGED - Keep all remaining methods as is
    ...
    # MODIFIED - Enhanced to handle new dimensionality reduction methods
    def analyze_optimal_solution(self, best_result):
        """
        Analyze the optimal solution in detail.
        Enhanced to provide component analysis for new DR methods.
        """
        ...
        if 'feature_mask' in best_result:
            ...
            # Ensure minimum features
            if np.sum(feature_mask) < self.min_features_per_cluster:
                indices = np.random.choice(np.where(~feature_mask)[0], 
                                         self.min_features_per_cluster - np.sum(feature_mask),
                                         replace=False)
        ...
        clustering_params = best_result.get('clustering_params', {'algorithm': 'kmeans'})
        preprocessing_params = best_result.get('preprocessing_params', {'scaler': 'robust'})
        ...
        # Add component analysis for different DR methods
        if preprocessing_params.get('pca_variance'):
            # Standard PCA component analysis
            pca = PCA().fit(X_optimal)
            components = pca.components_
            selected_features = [feature_names[i] for i in np.where(feature_mask)[0]]
            
            component_analysis = []
            for i, component in enumerate(components[:10]):  # Top 10 components
                top_indices = np.argsort(np.abs(component))[::-1][:10]
                top_features = [(selected_features[j], component[j]) for j in top_indices]
                component_analysis.append({
                    'component': i+1,
                    'explained_variance': pca.explained_variance_ratio_[i] if i < len(pca.explained_variance_ratio_) else 0,
            ...
            # Sparse PCA component analysis
            analysis['sparse_pca_info'] = {
                'n_components': preprocessing_params['sparse_pca'],
                'method': 'Sparse PCA promotes sparsity in loadings'
            }
        
        elif preprocessing_params.get('ica'):
            # ICA component analysis
            analysis['ica_info'] = {
                'n_components': preprocessing_params['ica'],
                'method': 'Independent components (statistically independent)'
            }
        
        # Add feature diversity metrics
        if len(selected_features) > 1:
            X_selected = X_optimal
            corr_matrix = np.corrcoef(X_selected.T)
            avg_correlation = np.mean(np.abs(corr_matrix[np.triu_indices_from(corr_matrix, k=1)]))
            analysis['feature_diversity'] = {
